...
def exam():
    ...
    info = input_group("enter your info before starting the exam",[
  input('Input your name', name='name',type = 'text',validate = validate_name),
  input('Input your prn', name='prn', type='text',)
])
    ...
    if c > 3:
        message = [style(put_html(
            "<h1 style='display:inline;border-bottom:0px'>Congratulations !! </h1>" + name + ", your score is <b>" + str(
                c) + "</b><br><br>"), 'color:green;'), style(put_html("<p>Result : <b>PASSED</b></p>"), 'color:green'),
                   put_html("<b>Thank You for your participation.</b>")]
        popup("Result", content=message, size='large', implicit_close=True, closable=True)
    else:
        message = [style(put_html(
            "<h1 style='display:inline;border-bottom:0px'>Oops! " + "</h1>" + name + ", your score is <b>" + str(
                c) + "</b><br><br>"), 'color:red'), style(put_html("<p>Result : <b>FAILED</b></p>"), 'color:red'),
                   put_html("<b>Thank You for your participation.</b><br><br>"), style(put_link('Retry ↺', ""), 'color:red;align-content: center;border-radius: 5px;color:#f9faf8;padding: 5px 100px;text-align:center;align-items : center;background-color: white;\
...
def validate_prn(prn):
    name = name.replace(" ", "")
    if (name == ""):
        return ("Please enter a non empty prn")
